...
/*:
 ...
 property is stored and the code that defines a property.
 ...
 A property wrapper can expose additional functionality by defining a projected
 value.
 
 The name of the projected value is the same as the wrapped value, except it
 ...
 */
func funcWithimplementationDetailPropertyWrapper(@SmallNumber(maximum: 5) value: Int) {
...
funcWithimplementationDetailPropertyWrapper(value: 6)
/*:
 ### API-level property wrappers
 Property wrappers that declare an `init(projectedValue:)` initializer are
 inferred to be API-level wrappers. These
 wrappers become part of the function signature, and the property wrapper is
 initialized at the call-site of the function.
 */
@propertyWrapper
...
func funcWithAPILevelPropertyWrapper(@APILevelPropertyWrapper value: Int) {
/*:
 The compiler generates:
 
 `func funcWithAPILevelPropertyWrapper(value _value: APILevelPropertyWrapper)`
 */
...
// init(wrappedValue:)
funcWithAPILevelPropertyWrapper(value: 10)

// init(projectedValue:)
funcWithAPILevelPropertyWrapper($value: APILevelPropertyWrapper(wrappedValue: 10))
//: Unapplied function references
let _: (Int) -> () = funcWithAPILevelPropertyWrapper
let _: (APILevelPropertyWrapper) -> () = funcWithAPILevelPropertyWrapper($value:)
/*:
...
/*:
 The property-wrapper attribute is not necessary if the backing property
 wrapper and the projected value have the same type
*/
